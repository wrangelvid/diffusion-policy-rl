...
class ManiDiffusionState(BaseLowdimDataset):
...
class ManiDiffusionRGBD(BaseImageDataset):
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        data = {
            ...
            'rgbd': self.replay_buffer['rgbd'],
            'state': self.replay_buffer['state'],
        }
        normalizer = LinearNormalizer()
        normalizer.fit(data=data, last_n_dims=1, mode=mode, **kwargs)
        normalizer['rgbd'] = get_image_range_normalizer()
        return normalizer
